...
def test(model, epoch, data_loader, res_fpath=None, visualize=False):
    ...
    if res_fpath is not None:
        ...
        for dataset_idx,dataset_name in enumerate(data_loader.dataset.dataset_list):
            ...
            np.savetxt(os.path.abspath(os.path.dirname(res_fpath)) + '/test_'+ str(epoch) +'.txt', res_list, '%d')
            
            lrecall, lprecision, lmoda, lmodp = evaluate(os.path.abspath(os.path.dirname(res_fpath)) + '/test_'+ str(epoch) +'.txt',
