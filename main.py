...
def train(model, epoch, data_loader, optimizer, log_interval, scheduler=None):
    ...
    for batch_idx, (data, map_gt, _, dataset_name) in enumerate(data_loader):
        data = data.to(device)
        optimizer.zero_grad()
        map_res = model(train_dataset, dataset_name, data, ignore_cam, duplicate_cam,args.dropview, args.train_cam)
...
def test(model, epoch, data_loader, res_fpath=None, visualize=False):
    ...
    accuracy_s = AverageMeter()
    all_res_list = []
    for batch_idx, (data, map_gt, frame, dataset_name) in enumerate(data_loader):
        data = data.to(device)
        
        with torch.no_grad():
            map_res = model(test_dataset, dataset_name, data, 0, 0, False, args.test_cam)
            #print(frame)
        ...
        if res_fpath is not None:
            ...
            #print(data_loader.dataset.dicts[dataset_name[0]]['base'].base.indexing)
            if data_loader.dataset.dicts[dataset_name[0]]['base'].base.indexing == 'xy':
                grid_xy = grid_ij[:, [1, 0]]
            else:
                grid_xy = grid_ij 
            all_res_list.append(torch.cat([torch.ones_like(v_s) * frame, grid_xy.float() *
                                           data_loader.dataset.dicts[dataset_name[0]]['base'].grid_reduce, v_s, torch.ones_like(v_s)*torch.tensor(data_loader.dataset.dataset_list.index(dataset_name[0]))], dim=1))
    ...
    if visualize and epoch!=0:
        ...
        subplt1.imshow(criterion.target_transform_(map_res, map_gt, data_loader.dataset.dicts[dataset_name[0]]['base'].map_kernel).detach().cpu().numpy().squeeze())
        plt.tight_layout()
    ...
    if res_fpath is not None:
        ...
        for dataset_idx,dataset_name in enumerate(data_loader.dataset.dataset_list):
            ...
            for frame in np.unique(all_res_list_temp[:, 0]):
            ...
            res_map_grid = res_list[res_list[:, 0] == frame, 1:]
            np.savetxt(os.path.abspath(os.path.dirname(res_fpath)) + '/test_'+ dataset_name +'_'+ str(epoch) +'.txt', res_list, '%d')
            
            lrecall, lprecision, lmoda, lmodp = evaluate(os.path.abspath(os.path.dirname(res_fpath)) + '/test_'+ dataset_name +'_'+ str(epoch) +'.txt',
            ...
            #print(f'####### moda: {lmoda:.1f}%, modp: {lmodp:.1f}%, prec: {lprecision:.1f}%, recall: {lrecall:.1f}% #######')
    ...
    for ij in res_map_grid:
        ...
        if data_loader.dataset.dicts[dataset_name]['base'].base.indexing == 'xy':
            ...
            map_res_nms[i, j] = 1
    map_res_nms = _traget_transform(torch.from_numpy(map_res_nms).unsqueeze(0).unsqueeze(0).float(), data_loader.dataset.dicts[dataset_name]['base'].map_kernel)
    map_res_nms = F.interpolate(map_res_nms, data_loader.dataset.dicts[dataset_name]['base'].reducedgrid_shape).squeeze().numpy()
    map_res_nms = np.uint8(255.0*map_res_nms)
    ...
    if visualize and epoch!=0:
        ...
        subplt0.imshow(map_res.detach().cpu().numpy().squeeze())
        subplt1.imshow(criterion.target_transform_(map_res, map_gt, data_loader.dataset.dicts[dataset_name]['base'].map_kernel).detach().cpu().numpy().squeeze())
    ...
    return losses / len(data_loader), precision_s.avg * 100, recall_s.avg * 100, accuracy_s.avg * 100, np.average(moda)
...
if __name__ == '__main__':
    ...
    elif 'gmvd_test' in args.dataset:
        data_path = os.path.expanduser('./GMVD_Dataset/test/gta_scene5')
        base1 = MultiviewX(data_path, args.cam_set, args.train_cam, args.test_cam)
        data_path = os.path.expanduser('./GMVD_Dataset/test/gta_scene6')
        base2 = MultiviewX(data_path, args.cam_set, args.train_cam, args.test_cam)
    elif 'gmvd_train' in args.dataset:
        data_path = os.path.expanduser('./GMVD_Dataset/train/gta_scene1_A')
        base1 = MultiviewX(data_path, args.cam_set, args.train_cam, args.test_cam)
        data_path = os.path.expanduser('./GMVD_Dataset/train/gta_scene1_B')
        base2 = MultiviewX(data_path, args.cam_set, args.train_cam, args.test_cam)
        data_path = os.path.expanduser('./GMVD_Dataset/train/gta_scene2')
        base3 = MultiviewX(data_path, args.cam_set, args.train_cam, args.test_cam)
        data_path = os.path.expanduser('./GMVD_Dataset/train/gta_scene3_A')
        base4 = MultiviewX(data_path, args.cam_set, args.train_cam, args.test_cam)
        data_path = os.path.expanduser('./GMVD_Dataset/train/gta_scene3_B')
        base5 = MultiviewX(data_path, args.cam_set, args.train_cam, args.test_cam)
        data_path = os.path.expanduser('./GMVD_Dataset/train/unity_scene1')
        base6 = MultiviewX(data_path, args.cam_set, args.train_cam, args.test_cam)
        data_path = os.path.expanduser('./GMVD_Dataset/train/unity_scene2')
        base7 = MultiviewX(data_path, args.cam_set, args.train_cam, args.test_cam)
        data_path = os.path.expanduser('./GMVD_Dataset/test/gta_scene5')
        base8 = MultiviewX(data_path, args.cam_set, args.train_cam, args.test_cam)
        data_path = os.path.expanduser('./GMVD_Dataset/test/gta_scene6')
        base9 = MultiviewX(data_path, args.cam_set, args.train_cam, args.test_cam)
    ...
    # Train and Test set
    if args.dataset == 'gmvd_train':
        
        train_dataset_gta_sc1_A = GetDataset(base1, train=True, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=1.0)

        train_dataset_gta_sc1_B = GetDataset(base2, train=True, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=1.0)

        train_dataset_gta_sc2 = GetDataset(base3, train=True, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=1.0)

        train_dataset_gta_sc3_A = GetDataset(base4, train=True, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=1.0)

        train_dataset_gta_sc3_B = GetDataset(base5, train=True, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=1.0)

        train_dataset_unity_sc1 = GetDataset(base6, train=True, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=1.0)

        train_dataset_unity_sc2 = GetDataset(base7, train=True, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=1.0)

        test_dataset_gta_sc5 = GetDataset(base8, train=False, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=0.0)
        
        test_dataset_gta_sc6 = GetDataset(base9, train=False, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=0.0)
        
        print("Training datasets")
        train_dataset = ConcatDataset(train_dataset_gta_sc1_A, train_dataset_gta_sc2, train_dataset_gta_sc3_A, train_dataset_unity_sc1, train_dataset_unity_sc2, train_dataset_gta_sc1_B, train_dataset_gta_sc3_B)
        print("Testing datasets")
        test_dataset = ConcatDataset(test_dataset_gta_sc5, test_dataset_gta_sc6)
        
    elif args.dataset == 'gmvd_test':
        train_dataset_sc5 = GetDataset(base1, train=True, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=1.0)
        test_dataset_sc5 = GetDataset(base1, train=False, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=0.0)
        train_dataset_sc6 = GetDataset(base2, train=True, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=1.0)
        test_dataset_sc6 = GetDataset(base2, train=False, transform=train_transform, grid_reduce=4, img_reduce=4, train_ratio=0.0)
        
        print("Training datasets")
        train_dataset = ConcatDataset(train_dataset_sc5, train_dataset_sc6)
        print("Testing datasets")
        test_dataset = ConcatDataset(test_dataset_sc5, test_dataset_sc6)
        ...
        test_dataset_ = GetDataset(base0, train=False, transform=train_transform, grid_reduce=4, img_reduce=4)
    ...
    model = MultiView_Detection(resnet_model, logdir, args.loss, args.avgpool, args.cam_set, len(args.train_cam))
